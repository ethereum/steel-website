...
## 2. Existing Testing Techniques and the Gap This Method Addresses
...
Property-based testing (for example [Hypothesis](https://hypothesis.readthedocs.io/)) defines invariants that should always hold, then generates random inputs that meet the preconditions and checks the invariants. This is useful for finding violations, but it answers a different question. It verifies that properties hold across random samples. It does not check that every distinct scenario was exercised. Passing one million property-based tests does not guarantee all equivalence classes were tested.
...
The Ethereum consensus specs are in a special position. They are executable Python code that *is* the authoritative specification, at once the reference that clients implement and the formal model of the protocol. Every client must reproduce these Python functions exactly. So specification-side techniques apply directly to the code, with no separate formal model: partition analysis, boundary values, and scenario enumeration.
...
## 3. Reading the Spec as a Testable Model {#sec-reading-spec}

Actual spec code shows the partition structure directly. [Section 4](#sec-method) gives the formal definitions of equivalence partitions, Disjunctive Normal Form, and the related concepts used here.
...
## 4. The Method: Two-Step Input-Space Analysis {#sec-method}
...
The output is a structured document (the "spec-doc") in a standard format. For `process_proposer_slashing`, the [spec-doc](https://notes.ethereum.org/@leolara/test_process_proposer_slashing) starts with the input fields table. The table goes beyond parameter types: it maps every state field the function touches, with its type, value range, cross-field constraints, and purpose:
...
The spec-doc then lists the six validation conditions (shown in [Section 3](#sec-reading-spec)), the call tree, and the key constants (`SLOTS_PER_EPOCH = 32`, `FAR_FUTURE_EPOCH = 2^64 - 1`, and so on). The call tree notes that `is_slashable_validator` inlines three sub-conditions and that `slash_validator` chains through `initiate_validator_exit` and balance updates.
...
Only 6 of 21 boundary values were tested explicitly, 29% explicit boundary coverage. Another 10 were covered implicitly (the test reaches the boundary by coincidence rather than by design). Five boundaries had no coverage. The implicit coverage is fragile: if a helper changes how it generates default values, the accidental boundary coverage is lost without warning.
...
## 6. Two Additional Functions: Generalization of the Method
...
The boundary analysis for this function is extensive. The deposit amount dimension has seven classes, each with boundary values: minimum deposit, typical, at the maximum effective balance, above the maximum effective balance, zero, below the minimum, and non-round (with extra gwei). The builder registry state dimension has boundaries at an empty registry, a single builder, and a registry near or at the `BUILDER_REGISTRY_LIMIT`. The pending deposits list has boundaries at empty, near the limit, and at the limit.
...
## 7. Scaling with AI Prompts {#sec-scaling-ai}

The spec-docs and eq-partition documents in this post were all generated with AI assistance. Each one states this at the top: "*This report is AI-generated and is not an authoritative source of truth.*" The human role was to give the right inputs and validate the outputs. The AI ran the procedure.

This works because the two-step method is algorithmic. It has well-defined inputs, well-defined outputs, and a clear procedure at each step. The expertise is in the method: the rules for finding partition dimensions from code constructs, listing classes, and computing boundary values. You do not need Ethereum domain knowledge to run it. An AI that has never seen a consensus spec can run the procedure correctly, if the instructions are precise enough.
...
Compare this with the request "write tests for this function." That asks the AI to decide *which scenarios are relevant*. This judgment depends on domain knowledge, threat modeling, and experience with real client bugs. The AI may produce plausible tests that cover the obvious cases and miss the subtle ones, with no systematic way to tell what was omitted. The two-step method removes this problem by making the "what to test" question mechanical and auditable, and then reducing the "write the test" step to implementing a well-specified scenario.

### Limitations

The method has real limits. **Opaque functions** are one. BLS signature verification, for example, becomes a boolean predicate in the analysis: valid or invalid. This is enough for enumerating scenarios (tests needed with valid and with invalid signatures), but the method cannot generate actual BLS test data. That needs cryptographic tooling, outside the partition analysis.

**Cross-function interactions** are another. The method analyzes one function at a time. Questions like how slashing interacts with epoch processing, or how withdrawal ordering affects later block validation, need system-level analysis that the function-level technique does not provide. [Section 9](#sec-granularity) takes up this question of scope in more detail. The method complements integration testing; it does not replace it.

## 8. The Loop Case: `process_attester_slashing` {#sec-loop-case}
...
## 9. Choosing the Right Granularity {#sec-granularity}

The three functions in this post share a trait: each is a single operation in the state transition, with a clear input and a clear output. This is the granularity where the method works best. Move up or down from it and the method meets limits worth naming.

**Moving up: whole-block functions.** A function like `on_block` does not validate one thing. It runs the full block through the state transition and the fork choice, calling dozens of smaller functions along the way. Its partition space is the cross-product of the partitions of everything it calls, so the number of combinations explodes. The result is too large to enumerate and too large to read. Fork choice is harder still, because it adds timing, reorganizations, equivocation, and the justification and finalization paths, each one a new family of variants. At this level the partition tables stop being a map and become noise.

**Moving down: helpers.** A helper like `is_slashable_validator` is small enough to partition in seconds, but two problems appear. First, clients rarely implement a helper as a separate unit. They inline it, or they fold its checks into a larger routine. A test vector that targets the helper in isolation aims at a seam that does not exist in client code. Second, the input classes the method derives for a helper, read on their own, include states the protocol never produces.

**The infeasibility caveat.** That second problem is not only about helpers. Take `process_withdrawals`. Analyzed alone, the method enumerates input states from the function's own checks. Some of those states are unreachable: by the time `on_block` calls `process_withdrawals`, the earlier steps of the transition have already constrained the state, and the invariants the protocol maintains rule out whole regions of the local input space. The method already removes infeasible combinations, but it does so with the function's own conditions, so it has no view of the global state. A class can be satisfiable for the function and still impossible for the chain. Analyzing one function at a time is what keeps the method tractable, and it is also what hides this constraint.

**The sweet spot.** The operation handlers sit at the right level. Their input is a pre-state plus an operation, which is the shape the consensus-spec test vectors already use and the shape every client consumes. The scenario count per function stays small enough to enumerate in full, and most of the classes the method derives map to states a real chain can present to the function. This is why `process_proposer_slashing`, `process_withdrawals`, and `process_deposit_request` were a good fit, and why the same enumeration would be impractical for `on_block` and of little value for a helper.

**A way to bridge the levels.** There is a direction that turns the unit-level work into block-level coverage and removes the infeasibility caveat at the same time. Once you have a scenario for an operation, you can work backwards: build a starting state and a sequence of blocks that drive the chain, through valid processing, into that scenario. The test then enters through `on_block`, the way a client runs in production, and the state it reaches is reachable by construction. I have not built this. It is a direction worth exploring, and it points from the operation-level scenarios the method already produces toward the block-level tests that are hardest to write by hand.

## 10. Implications for the Consensus-Specs Testing Ecosystem
...
This changes the agent's task from "decide what to test" to "implement this specific test scenario given these constraints." The first is a creative, error-prone judgment. The second is a mechanical, verifiable code generation task. The agent gets an instruction like: "Write `test_invalid_slots_same_epoch_different_slot`: `header_1.slot = epoch_start_slot`, `header_2.slot = epoch_start_slot + 1`, same `proposer_index`, headers differ in `body_root`. Expected: `AssertionError` on the slot equality check." This is a well-defined specification. The agent does not need to know why the scenario matters or how it relates to consensus safety. It needs to translate the specification into code that follows the existing test patterns in the codebase.

### A two-stage pipeline

The full workflow has two stages, each with a different balance of AI and human contribution. There is no prioritization step between them. At the operation level the scenario set per function is small, so the goal is the complete set, and an agent implements all of it at low cost.

In the first stage, **analyze**, the AI generates the spec-doc and eq-partition using the structured prompts from [Section 7](#sec-scaling-ai). The human reviews and validates the partition analysis, checking each dimension against the spec code and confirming the enumeration is complete. This is where errors in the analysis are caught: a missing dimension, a misclassified boundary, or an infeasible combination that should have been removed.

In the second stage, **implement**, an AI coding agent generates the test code for each gap, using the matching eq-partition row as its specification and the spec-doc as context. The agent follows the existing test patterns in the codebase: for `process_proposer_slashing`, the `prepare_process_proposer_slashing` and `assert_process_proposer_slashing` helpers, which structure each test's setup and expectations.

A purpose-built tool can do both stages at once. It reads the spec's syntax tree, derives the partition classes directly, and generates the test vectors from them, without a prompt at any step. This makes the whole pipeline deterministic: the same function always produces the same partitions and the same vectors. The human role does not change. The reviewer validates that the partition analysis matches the spec, and the tool handles the enumeration and the code generation.

The systematic method is what makes the agent trustworthy. The partition analysis is auditable: each dimension can be checked against the spec code. Each generated test targets a specific, well-defined scenario. So the human can verify coverage by reviewing the partition table instead of reading every line of generated test code. The review question shifts from "did the AI write good tests?" to "does the partition table correctly capture the function's input space?" The first is hard to answer. The second is straightforward.

### Relevance to hard fork testing at scale

Each Ethereum hard fork changes dozens of spec functions. The current process, experts writing tests function by function, is a bottleneck. The test-writing work for the three functions in this post needed weeks of iteration per function. With the two-stage pipeline, the analysis step can run across all changed functions in parallel. The partition tables show immediately which functions have the largest coverage gaps. AI agents can draft the test implementations for the gaps. Human reviewers focus on validating the partition logic, instead of writing boilerplate setup code.

This keeps human judgment and moves it to where it is most valuable, validating the partition analysis, while automating the mechanical parts. The goal is that every spec function has a documented, auditable, complete enumeration of its test scenarios, and that turning those scenarios into executable tests becomes the simpler part of the process.
...
The systematic output, structured partition tables with concrete gap descriptions and boundary values, is also exactly what AI coding agents need to generate test code reliably. The method changes the agent's task from "decide what to test" to "implement this specific scenario." The human reviews the partition table; the agent writes the code. This two-stage pipeline (analyze, implement) scales to the dozens of functions changed in each hard fork.
